...
async def send_verification_success(user_id, uid, chat_id=None, message_id=None):
    try:
        ...
        elif like_response.get("status") == 2:
            text = (
                "⚠️ *Maxed Likes Reached!*\n\n"
                f"❌ You already received the maximum likes for UID `{like_response.get('UID', uid)}`."
            )
        else:
...
def verify(code):
    try:
        ...
        if datetime.utcnow() > user["expires_at"]:
            return "❌ This verification link has expired. Please generate a new one.", 400
        ...
        return (
            ...
            f"<p>User ID: <b>{user['user_id']}</b><br>You may return to Telegram.</p>"
        )
...
async def like_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        msg = (
            ...
            f"🔗 {short_link}\n\n"
            f"⚠️ Link expires in 10 minutes and is valid for one-time use only."
        )
...
def run():
    ...
    # Run Flask in safe background thread
    threading.Thread(
